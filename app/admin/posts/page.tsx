      ...
      <Link
        ...
        className={`px-3 py-1.5 rounded text-xs tracking-widest uppercase border transition-all ${
          active
            ...
            : "border-[#c9a227]/30 text-[#f0e6c8]/50 hover:border-[#c9a227]/60"
        }`}
      >
        {label}
      </Link>
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link
          href="/admin/post/new"
          className="bg-[#c9a227] hover:bg-[#e2b84e] text-[#0d1a08] font-bold px-4 py-2 rounded text-xs tracking-widest uppercase transition-colors"
        >
          + New Post
        </Link>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        <span className="w-px h-6 bg-[#c9a227]/20 self-center" />
      ...
      <div className="bg-[#1a2e10] border border-[#c9a227]/20 rounded-lg overflow-hidden">
        {posts.length === 0 ? (
          <p className="text-center py-12 text-[#f0e6c8]/40">No posts found.</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#c9a227]/20 text-xs tracking-widest uppercase text-[#f0e6c8]/40">
                ...
                <th className="text-left px-3 py-3 hidden lg:table-cell">
                  <Link href={filterHref({ sort: "created" })} className="hover:text-[#c9a227]">
                    Created
                  </Link>
                </th>
                <th className="text-left px-3 py-3 hidden lg:table-cell">
                  <Link href={filterHref({ sort: "published" })} className="hover:text-[#c9a227]">
                    Published
                  </Link>
                </th>
            ...
            <tbody>
              {posts.map((post) => (
                <tr
                  key={post.id}
                  className="border-b border-[#c9a227]/10 last:border-0 hover:bg-[#0d1a08]/30 transition-colors"
                >
                  <td className="px-5 py-3">
                    <span className="font-medium text-[#f0e6c8]/90 line-clamp-1">{post.title}</span>
                  </td>
                  ...
                  <td className="px-3 py-3 hidden lg:table-cell text-[#f0e6c8]/40 text-xs">
                    {new Date(post.createdAt).toLocaleDateString()}
                  </td>
                  <td className="px-3 py-3 hidden lg:table-cell text-[#f0e6c8]/40 text-xs">
                    {post.publishedAt ? new Date(post.publishedAt).toLocaleDateString() : "—"}
                  </td>
                  <td className="px-3 py-3 hidden md:table-cell text-[#f0e6c8]/40 text-xs">
                  ...
                  <td className="px-5 py-3 text-right">
                    <div className="flex items-center justify-end gap-3">
                      <Link
                        ...
                        className="text-xs text-[#c9a227] hover:text-[#e2b84e] tracking-widest uppercase transition-colors"
                      >
                        Edit
                      </Link>
                      {post.status === "PUBLISHED" && (
                        <Link
                          href={`/post/${post.slug}`}
                          target="_blank"
                          className="text-xs text-[#f0e6c8]/30 hover:text-[#f0e6c8] tracking-widest uppercase transition-colors"
                        >
                          View
                        </Link>
                      )}
